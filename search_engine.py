...
def _score_pigment_against_target(target: Tuple[float, float, float], rm: RawMaterial) -> float:
...
def _build_km_pigment_pool(eligible_rms: List[RawMaterial]) -> List[Tuple[RawMaterial, Pigment]]:
    km_corr = get_km_corrections()
    pool: List[Tuple[RawMaterial, Pigment]] = []
    for rm in eligible_rms:
        ...
        pool.append((
            ...
            Pigment(
                ...
                ks_corrections=km_corr.get(rm.rawmaterialid, (1.0, 1.0, 1.0)),
            )
        ))
    return pool


def _blend_result_from_components(
    blend_type: str,
    source: str,
    target: Tuple[float, float, float],
    pigment_rows: List[Tuple[RawMaterial, Pigment, float]],
    ...
    for rm, _, conc in pigment_rows:
        ...
        pigments.append({
            ...
            "heat_resistance": rm.heat_resistance,
            "light_fastness_tone": rm.light_fastness_tone,
            "weather_fastness_tone": rm.weather_fastness_tone,
        })
    result = {
        "type": blend_type,
        "source": source,
        "pigments": pigments,
        "carrier_kg": round(max(0.0, 100.0 - total_pct), 3),
        "predicted_lab": {
            "L": round(predicted_lab[0], 3),
            "a": round(predicted_lab[1], 3),
            "b": round(predicted_lab[2], 3),
        },
        "target_lab": {
            "L": target[0],
            "a": target[1],
    ...
    if score is not None:
        result["objective_score"] = round(score, 6)
...
def search_recipes(
    ...
    # -----------------------------------------------------------------------
    # Step 4: K-M pigment combination suggestions (two-stage: heuristic → optimizer)
    # fast_mode=True returns heuristic-only results for real-time UI previews;
    # the default full pipeline refines concentrations via continuous optimisation.
    # -----------------------------------------------------------------------
    pigment_suggestions = _two_stage_suggest(
...
def _suggest_pigment_combinations(
    ...
    # ---------------------------------------------------------------------------
    # Helper builder — delegates to the shared rich-schema builder so that
    # heuristic and optimizer results are structurally identical.
    # ---------------------------------------------------------------------------
    def _make_result_rich(
        rows: List[Tuple[RawMaterial, Pigment, float]],
        pred: Tuple[float, float, float],
        de: float,
        rtype: str,
    ) -> Dict:
        r = _blend_result_from_components(
            blend_type=rtype,
            source="heuristic",
            target=target,
            pigment_rows=rows,
        ...
        r["carrier_label"] = f"{polymer} base resin"
        return r
    ...
    for rm, pig in candidates:
        ...
        for c in conc_main:
            try:
                ...
                continue
        if best_pred:
            single_results.append(_make_result_rich([(rm, pig, best_c)], best_pred, best_de, "single"))
    ...
    for i in range(len(candidates)):
        for j in range(i + 1, len(candidates)):
            rm1, pig1 = candidates[i]
            rm2, pig2 = candidates[j]
            ...
            for c1 in conc_main:
                for c2 in conc_mod:
                    try:
                        ...
                        continue
            if best_pred:
                two_pig_results.append(_make_result_rich(
                    [(rm1, pig1, best_c1), (rm2, pig2, best_c2)],
    ...
    rm_by_name: Dict[str, RawMaterial] = {pig.name: rm for rm, pig in candidates}
    ...
    for combo in two_pig_results[:5]:
        ...
        rm1 = rm_by_name.get(p1_entry["name"])
        rm2 = rm_by_name.get(p2_entry["name"])
        if pig1 is None or pig2 is None or rm1 is None or rm2 is None:
            continue
        names_in = {p1_entry["name"], p2_entry["name"]}
        base_de = combo["delta_e"]

        for rm3, pig3 in candidates:
            ...
            if best_pred3 and best_de3 < base_de * 0.85:
                three_pig_results.append(_make_result_rich(
                    [(rm1, pig1, c1), (rm2, pig2, c2), (rm3, pig3, best_c3)],
...
def _continuous_optimize_blends(
    target: Tuple[float, float, float],
    candidates: List[Tuple[RawMaterial, Pigment]],
    heuristic_seeds: Optional[List[Dict]] = None,
    top_n: int = 5,
    max_colorants: int = 3,
    seed: int = 42,
) -> List[Dict]:
    ...
    target_chroma = math.sqrt(target[1] ** 2 + target[2] ** 2)
    is_achromatic = target_chroma < 10
    max_total = 0.25 if is_achromatic else 0.35
    low = 0.0005
    rng = random.Random(seed)
    scored: List[Tuple[float, Dict]] = []
    seen = set()

    def objective(indices: Tuple[int, ...], concs: np.ndarray) -> Tuple[float, Tuple[float, float, float]]:
        clipped = np.clip(concs, low, max_total)
        total = float(clipped.sum())
        if total > max_total:
            clipped = clipped * (max_total / total)
        mixture = [(candidates[idx][1], float(clipped[pos])) for pos, idx in enumerate(indices)]
        pred = predict_mixture_lab(mixture)
        de = delta_e_cie2000(target, pred)
        return de, pred

    def refine(indices: Tuple[int, ...], initial: np.ndarray) -> Optional[Dict]:
        current = initial.astype(float)
        current = np.clip(current, low, max_total)
        total = float(current.sum())
        if total > max_total:
            current *= max_total / total
        best_de, best_pred = objective(indices, current)
        best_x = current.copy()

        def scalar_objective(x):
            x = np.clip(np.asarray(x, dtype=float), low, max_total)
            total_x = float(x.sum())
            penalty = 0.0
            if total_x > max_total:
                penalty = ((total_x - max_total) * 500.0)
            de, _ = objective(indices, x)
            return de + penalty
        ...
        try:
            res = minimize(
                ...
                current,
                method="Powell",
                bounds=[(low, max_total) for _ in indices],
                options={"maxiter": 120, "xtol": 1e-4, "ftol": 1e-4},
            )
            trial = np.clip(np.asarray(res.x, dtype=float), low, max_total)
            total = float(trial.sum())
            if total > max_total:
                trial *= max_total / total
            trial_de, trial_pred = objective(indices, trial)
            if trial_de < best_de:
                best_x = trial
                best_de = trial_de
        ...
        signature = _normalized_signature(indices, best_x)
        if signature in seen:
            return None
        seen.add(signature)
        rows = [
            (candidates[idx][0], candidates[idx][1], float(best_x[pos]))
            for pos, idx in enumerate(indices)
            if best_x[pos] > low
        ]
        return _blend_result_from_components(
            blend_type="continuous_optimization",
            source="optimizer",
            target=target,
            pigment_rows=rows,
            predicted_lab=best_pred,
            delta_e=best_de,
            score=best_de,
    ...
    name_to_idx = {rm.rawmaterialname: idx for idx, (rm, _) in enumerate(candidates[:candidate_count])}
    seed_vectors: List[Tuple[Tuple[int, ...], np.ndarray]] = []

    for item in heuristic_seeds or []:
        mapped = []
        for pigment in item.get("pigments", []):
            idx = name_to_idx.get(pigment.get("name"))
            conc = pigment.get("concentration")
            if idx is None or conc is None:
                continue
            mapped.append((idx, float(conc)))
        if not mapped:
            continue
        mapped.sort(key=lambda x: x[0])
        indices = tuple(idx for idx, _ in mapped[:max_colorants])
        vector = np.array([conc for _, conc in mapped[:max_colorants]], dtype=float)
        seed_vectors.append((indices, vector))

    for idx in candidate_indices:
        init = np.array([0.02 if is_achromatic else 0.05], dtype=float)
        result = refine((idx,), init)
        if result:
            scored.append((result["delta_e"], result))

    for i in range(candidate_count):
        for j in range(i + 1, candidate_count):
            init = np.array([0.04, 0.015] if is_achromatic else [0.06, 0.02], dtype=float)
            result = refine((i, j), init)
            if result:
                scored.append((result["delta_e"], result))

    for indices, vector in seed_vectors:
        result = refine(indices, vector)
        if result:
            scored.append((result["delta_e"], result))

    subset_budget = 36
    for _ in range(subset_budget):
        size = rng.choice([2, 3] if max_colorants >= 3 else [2])
        if candidate_count < size:
            continue
        subset = tuple(sorted(rng.sample(candidate_indices, size)))
        base = np.array([rng.uniform(low, max_total / size) for _ in subset], dtype=float)
        result = refine(subset, base)
        if result:
            scored.append((result["delta_e"], result))
